...
import { fetchSistemas } from "@/lib/api";
...
export const Software = () => {
  const { data: dbSystems } = useQuery({ queryKey: ["sistemas"], queryFn: fetchSistemas });

  const systems = dbSystems?.length
    ? dbSystems.map((s: Record<string, unknown>) => ({ ...s, icon: getIcon(s.icone as string) }))
    : fallbackSystems;
    ...
    <section id="softwares" className="py-24 bg-secondary/40">
      <div className="container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center max-w-2xl mx-auto mb-16">
          <span className="inline-block text-primary font-semibold text-sm uppercase tracking-wider mb-4">Sistemas Fiorilli</span>
          <h2 className="text-4xl md:text-5xl font-bold text-foreground leading-tight mb-4">
            Soluções completas para sua <span className="text-gradient-primary">prefeitura</span>
          </h2>
          <p className="text-muted-foreground text-lg">Implantamos, treinamos e damos suporte aos principais sistemas Fiorilli usados na administração pública.</p>
        </motion.div>
